...
def get_schema(dataset,metadata=None):
    ...
    #         ' You can generate it on an existing dataset using petastorm-generate-metadata.py')
    if hasattr(dataset, "common_metadata"):
        dataset_metadata_dict = dataset.common_metadata.metadata
    else:
        dataset_metadata_dict = metadata.metadata
...
def infer_or_load_unischema(dataset,metadata = None):
    ...
    try:
        return get_schema(dataset,metadata)
    except PetastormMetadataError:
