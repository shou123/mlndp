...
class CustomDataset(Dataset):
    ...
    def __getitems__(self, idx):
        ...
        end_idx = self.current_batch_index + length
        self.current_batch_index += length


        end_idx = min(end_idx, len(self.record_batches[0]['image']))

        batch_image_data = self.record_batches[0]['image'].slice(offset = start_idx, length = length).to_pandas()
        batch_label_data = self.record_batches[0]['label'].slice(offset = start_idx, length = length).to_pandas()
        # print(f"batch_label_data: {batch_label_data}")

        for i in range(length):
            image_tensor = torch.FloatTensor(batch_image_data[i]).view(1, 28, 28)  # Adjust the shape as needed
            target = int(batch_label_data.iloc[i])

            batch_data.append({'data': image_tensor, 'target': target})
        return batch_data
...
#===============================Skyhook=========================================
    ...
    train_loader = torch.utils.data.DataLoader(train_custom_dataset, batch_size=32, shuffle=False)
